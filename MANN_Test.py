...
if __name__ == '__main__':
    # hash_bit = 144
    # accuracy = open("test_acc_20way5shot.txt", "a+")
    # for hash_bit in range(544, 545, 32):
    # print('hashing bit: ', hash_bit, file=accuracy)
    # print('hashing bit: ', hash_bit)
    ...
    for step in range(args.epoch):
        ...
        # print(LSH_a, LSH_b)
        for _ in range(10):    # 200*50=10000 batches task
            # test3ew-
            ...
            # variation = 4 * torch.randn(args.n_way, hash_bit*2).cuda()
            # split to single task each time
            for x_spt_one, y_spt_one, x_qry_one, y_qry_one in zip(x_spt, y_spt, x_qry, y_qry):
                #print(x_spt_one.shape, y_spt_one.shape, x_qry_one.shape, y_qry_one.shape)
                #print(y_spt_one)

                #test_acc = maml.test_lsh(x_spt_one, y_spt_one, x_qry_one, y_qry_one, LSH_a, LSH_b)
                test_acc = maml.test_quantize(x_spt_one, y_spt_one, x_qry_one, y_qry_one)
                #test_acc = maml.test_quantize_RRAM(x_spt_one, y_spt_one, x_qry_one, y_qry_one, variation)
        ...
        # print('Mean Test Acc:', mean_accs, file = accuracy)
    # accuracy.close()
