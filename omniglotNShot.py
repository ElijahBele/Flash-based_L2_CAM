    ...
    def load_data_cache(self, data_pack):
        ...
        for sample in range(50):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                selected_cls = np.random.choice(data_pack.shape[0], self.n_way, False)#def choice(a, size=None, replace=True, p=None)
                ...
                for j, cur_class in enumerate(selected_cls):
                    ...
                    y_spt.append([j for _ in range(self.k_shot)]) #此处实际用作训练的数据标签取值为0,1,2,3,4
                    y_qry.append([j for _ in range(self.k_query)])

                # shuffle inside a task
